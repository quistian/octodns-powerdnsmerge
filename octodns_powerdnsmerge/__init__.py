...
class PowerDnsMergeBaseProvider(BaseProvider):
    SUPPORTS_GEO = False
    ...
    def list_zones(self):
        ...
        for zone_data in resp.json():
            leaf_zone = zone_data['name']
            toks = leaf_zone.split('.')
            hrid = toks.pop(0)
            ms_zone = '.'.join(toks)
            if ms_zone in hrid_dict:
                hrid_dict[ms_zone].append(hrid)
            else:
                hrid_dict[ms_zone] = [hrid]
        self.hrid_dict = hrid_dict
        return(sorted(hrid_dict.keys()))
    ...
    def populate(self, zone, target=False, lenient=False):
        ...
        hrid_dict = self.hrid_dict
        hrids = hrid_dict[zone.name]
        hrid = hrids.pop()
        resp = None
        try:
            resp = self._get(f'zones/{hrid}.{zone.name}')
        ...
        if resp:
            self.log.debug(f'resp: {resp.json()}')
            exists = True
            ...
            for rrset in rrsets:
                _type = rrset['type']
                _provider_specific_type = f'PowerDnsMergeProvider/{_type}'
                if (
                ...
                toks = rrset['name'].split('.')
                record_name = toks[0]
                hrid = toks.pop(1)
                # hostname has to match hrid number in zone name
                if hrid != record_name[1:4]:
                    continue
                newname = ".".join(toks)
                rrset['name'] = newname
                record = Record.new(
...
class PowerDnsMergeProvider(PowerDnsMergeBaseProvider):
    def __init__(
        self,
        id,
        host,
        ...
        **kwargs,
        ...
        self.log = logging.getLogger(f'PowerDnsMergeProvider[{id}]')
        self.log.debug(
